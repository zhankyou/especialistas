...
class RespiratoriaService:
    ...
    def save_registro(data: dict, especialista_email: str) -> dict:
        try:
            ...
            expediente = db.session.get(FormularioRespiratoria, record_id)
            is_new = False
            if not expediente:
                expediente = FormularioRespiratoria(id=record_id)
            ...
            # Procesamiento de Arreglo de Evidencias
            ...
            if evidencias_payload and isinstance(evidencias_payload, list):
                ...
                nom_prof = str(
                    data.get('nombre_profesional', getattr(expediente, 'nombre_profesional', 'NoName'))).replace(' ',
                                                                                                                 '_').strip()
                ...
                base_filename = f"{territorio}_{microterritorio}_{nom_prof}_{fecha_visita}_{cod_fam}_TR"
                ...
                for idx, file_obj in enumerate(evidencias_payload):
                    ...
                    try:
                        url = DriveService.upload_base64_evidence(base64_data, filename, "respiratoria")
            ...
            # Mapeo del resto de columnas protegiendo las Firmas en DB
            ...
            print(f"INFO: Expediente Respiratoria {record_id} {action} exitosamente.")
            ...
            db.session.rollback()
            print(f"Error DML Respiratoria: {str(e)}")
