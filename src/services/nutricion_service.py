    ...
    def save_registro(data: dict, especialista_email: str) -> dict:
        try:
            ...
            if codigo_familia and not re.match(r'^F\d{4}$', codigo_familia):
                return {"status": "error", "message": "Codigo de Familia invalido. Formato requerido: F0000.",
                        "code": 400}

            expediente = db.session.get(FormularioNutricion, record_id)
            is_new = False
            if not expediente:
                expediente = FormularioNutricion(id=record_id)
            ...
            if evidencias_payload and isinstance(evidencias_payload, list):
                ...
                nom_prof = str(
                    data.get('nombre_nutricionista', getattr(expediente, 'nombre_nutricionista', 'NoName'))).replace(
                    ' ', '_').strip()
                ...
                base_filename = f"{territorio}_{microterritorio}_{nom_prof}_{fecha_visita}_{cod_fam}_NUT"
                ...
                for idx, file_obj in enumerate(evidencias_payload):
                    ...
                    try:
                        url = DriveService.upload_base64_evidence(base64_data, filename, "nutricion")
            ...
            print(f"INFO: Expediente Nutricion {record_id} {action} exitosamente.")
            ...
            db.session.rollback()
            print(f"Error DML Nutricion: {str(e)}")
