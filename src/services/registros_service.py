...
class RegistrosService:
    ...
    Inspecciona dinamicamente las columnas fisicas de PostgreSQL Aiven para evitar
    errores UndefinedColumn y construye un UNION ALL resiliente a variaciones de esquema.
    """

    @classmethod
    def _resolve_column(cls, existing_cols: set, candidate_list: list, default_value: str = "'N/A'") -> str:
        """
        Retorna la primera columna que exista en la tabla o un valor por defecto.
        """
    ...
    def _build_table_select(cls, table_name: str, modulo_name: str, existing_cols: set) -> str:
        """
        Construye la clausula SELECT dinamica adaptada a las columnas reales de cada tabla.
        """
        ...
        prof_candidates = ['nombre_nutricionista', 'nombre_fisio', 'nombre_profesional', 'profesional_email', 'especialista_email', 'email']
        prof_expr = cls._resolve_column(existing_cols, prof_candidates, "'sistema'")

        fecha_expr = cls._resolve_column(existing_cols, ['fecha_visita', 'created_at', 'fecha'], "NOW()::text")

        if 'is_deleted' in existing_cols:
            deleted_expr = "COALESCE(is_deleted, false)"
        else:
            deleted_expr = "false"

        if 'created_at' in existing_cols:
            created_expr = "COALESCE(created_at, NOW())"
        elif 'fecha_visita' in existing_cols:
            created_expr = "COALESCE(fecha_visita::timestamp, NOW())"
        else:
            created_expr = "NOW()"
        ...
        return f"""
            SELECT 
                ...
                COALESCE(NULLIF(TRIM(LOWER({prof_expr})), ''), 'sistema') AS especialista_email,
    ...
    def get_all_records(cls, user_data: dict, is_deleted: bool = False, search_query: str = ""):
        ...
        try:
            ...
            print(f"[DATA SERVICE ERROR] Fallo al consolidar tablas de especialidad: {str(e)}")
            ...
            with db.engine.begin() as conn:
                for table in tables:
                    if user_role in ['ADMINISTRADOR', 'COORDINADOR']:
                        query = text(f"UPDATE {table} SET is_deleted = true WHERE id = :id")
    ...
    def restore_record(cls, record_id: str, user_data: dict):
        ...
        try:
            with db.engine.begin() as conn:
                for table in tables:
                    query = text(f"UPDATE {table} SET is_deleted = false WHERE id = :id")
                    res = conn.execute(query, {"id": record_id})
                    mutations += res.rowcount
    ...
    def export_csv(cls, user_data: dict, filter_type: str, p1: str, p2: str):
        ...
        writer.writerow(['ID', 'Modulo', 'Codigo Familia', 'Jefe de Hogar', 'Documento Identidad', 'Especialista Email', 'Fecha Visita'])
